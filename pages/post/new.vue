<template>
  <UCard class="w-full mt-2">
    ...
    <div class="flex flex-col my-2">
      <UForm ref="form" :schema="createPostSchema" :state="state" :validate-on="['submit']" class="space-y-4" @submit="onSubmit" autocomplete="off">
        <UFormGroup label="标题" name="title">
          <UInput v-model="state.title" autocomplete="off" />
        </UFormGroup>
        <UFormGroup label="标签" name="tags">
          <USelectMenu value-attribute="id" option-attribute="desc" v-model="state.tagId" :options="tags"> </USelectMenu>
        </UFormGroup>
        <UFormGroup label="正文" name="content">
          <client-only>
            <MdEditor :theme="mode as any" @onUploadImg="onUploadImg" style="max-height: 400px" v-model="state.content" :preview="false" :toolbars="toolbars" editor-id="newPost">
        ...
        <div>
          <UButton type="submit" :loading="pending"> 发表(Ctrl+Enter提交) </UButton>
        </div>
      </UForm>
    </div>
  </UCard>
