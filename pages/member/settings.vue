<template>
  <UCard class="w-full mt-2 text-sm">
    <template #header>
      <div class="flex gap-1 items-center"><UIcon name="i-carbon-settings" title="设置" class="size-4" /><span class="font-semibold">个人设置</span></div>
    </template>
    <UForm :schema="saveSettingsRequestSchema" :state="state" class="space-y-4" @submit="onSubmit">
      ...
      <UFormGroup label="自定义签名" name="css">
        <template #hint> 2级以上用户可以添加,只支持markdown语法的链接写法,不支持其它格式,比如图片等 </template>
